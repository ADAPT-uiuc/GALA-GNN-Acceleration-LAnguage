...
GALAGNN(int size0, int size1, int size2, int size3, int size4, int size5, int size6, int size7, int size8){
fc0 = register_module("fc0", torch::nn::Linear(size0, size1));
fc1 = register_module("fc1", torch::nn::Linear(size1, size2));
fc2 = register_module("fc2", torch::nn::Linear(size2, size3));
fc3 = register_module("fc3", torch::nn::Linear(size3, size4));
fc4 = register_module("fc4", torch::nn::Linear(size4, size5));
fc5 = register_module("fc5", torch::nn::Linear(size5, size6));
fc6 = register_module("fc6", torch::nn::Linear(size6, size7));
fc7 = register_module("fc7", torch::nn::Linear(size7, size8));
   }
...
int main(int argc, char **argv) {
  ...
  auto options_float_tile = 
    ...
    SM adj0;
    std::string filename = "/shared/damitha2/gala_npy/RedditDataset/";
...
int num_iters = 100;
auto net = std::make_shared<GALAGNN>(602, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 41);net->to(device);torch::optim::Adam optimizer(
