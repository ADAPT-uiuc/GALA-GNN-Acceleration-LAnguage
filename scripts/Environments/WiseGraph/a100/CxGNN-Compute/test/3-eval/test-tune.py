...
def show_tune_stages_gcn():
    ...
    cxgc.prof("cxg", "tune", lambda: cxgnncomp.AggrOP.apply(x, ptr, idx, ptr.shape[0] - 1))


    dgl_conv = dglnn.GraphConv(infeat, infeat, allow_zero_in_degree=True).cuda()
    dgl_block = dgl.create_block(('csc', (ptr, idx, torch.tensor([]))),
                                int(ptr.shape[0] - 1), int(ptr.shape[0] - 1))
    # dgl
...
def tensor_centric_rgcn(x, weights, ptr, idx, rel, num_center):
    ...
    for i in range(num_rel):
        ...
        cxgnncomp_backend.selective_aggr(transformed_x, ptr, idx, (rel == i), output,
                        num_center)
    return output
...
def show_tune_stages_rgcn():
    ...
    t = cxgc.prof("tensor", "conv", lambda: tensor_centric_rgcn(x, weight, ptr, idx, types, ptr.shape[0] - 1))[0]
    ans.append(t)
    ...
    cxgc.prof(
        "spmm", "origin", lambda: cxgnncomp_backend.sage_mean_forward(
            x_indexed, ptr, new_idx, ptr.shape[0] - 1))[0]
...
def run_dgl(x, ptr, idx, edge_index, lstm_module):
    ...
    return cxgc.prof("lstm", "dgl",
              lambda: dgl_graph.update_all(msg_fn, _lstm_reducer))[0]
...
def show_tune_stages_gat():
    ...
    # cxgnn
    ans.append(
        cxgnncomp.prof("cxgnn", "full", lambda: cxgnn_conv(x, ptr, idx, ptr.shape[0] - 1, ptr.shape[0] - 1, idx.shape[0]))[0])
    ...
    ans.append(
        cxgnncomp.prof("cxgnn", "aggr", lambda: cxgnncomp.AggrOP.apply(new_x, ptr, idx, ptr.shape[0] - 1))[0])
