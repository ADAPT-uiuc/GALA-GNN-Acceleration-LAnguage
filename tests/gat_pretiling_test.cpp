...
int main(int argc, char **argv) {
    ...
    int rows_per_tile = stoi(string(argv[4]));
    int cols_per_tile = stoi(string(argv[5]));
    ...
    GAT_PRETILE<SM_t, DMd_t, DMi_t> my_gat(tiled_adj,
                                           &input_emb,
                                           &labels,
                                           &train_masks,
                                           &valid_masks,
    ...
    for (i = 0; i < num_iters-1; i++)
        my_gat.forward_pass(false, rows_per_tile, false
    ...
    my_gat.forward_pass(true, rows_per_tile, false
#if defined(ST_1) || defined(ST_2)
            , cols_per_tile
#endif
#ifdef LO_KK
