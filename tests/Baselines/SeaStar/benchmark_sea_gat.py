...
from gala_gatconv_sea import TimedGATConv as TimedGraphConv
...
class EdgeGCNetwork(torch.nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList()
        if h_layers == 0:
            self.layers.append(TimedGraphConv(in_feats=in_feats, out_feats=out_feats, num_heads=1, activation=F.relu, allow_zero_in_degree=True))
        else:
            self.layers.append(TimedGraphConv(in_feats=in_feats, out_feats=n_hidden, num_heads=1, activation=F.relu, allow_zero_in_degree=True))
            for layer in range(h_layers - 1):
                self.layers.append(TimedGraphConv(in_feats=n_hidden, out_feats=n_hidden, num_heads=1, activation=F.relu, allow_zero_in_degree=True))
            self.layers.append(TimedGraphConv(in_feats=n_hidden, out_feats=out_feats, num_heads=1, allow_zero_in_degree=True))
