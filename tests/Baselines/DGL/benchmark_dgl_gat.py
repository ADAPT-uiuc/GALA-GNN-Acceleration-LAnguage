...
class GCNOpt(nn.Module):
    def __init__(self,
        ...
        if h_layers == 0:
            self.layers.append(dglnn.GATConv(in_feats=in_feats, out_feats=out_feats, num_heads=1, bias=False, activation=F.relu, allow_zero_in_degree=False))
        else:
            self.layers.append(dglnn.GATConv(in_feats=in_feats, out_feats=n_hidden, num_heads=1, bias=False, activation=F.relu, allow_zero_in_degree=False))
            for layer in range(h_layers - 1):
                self.layers.append(dglnn.GATConv(in_feats=n_hidden, out_feats=n_hidden, num_heads=1, bias=False, activation=F.relu, allow_zero_in_degree=False))
            self.layers.append(dglnn.GATConv(in_feats=n_hidden, out_feats=out_feats,  num_heads=1, bias=False, allow_zero_in_degree=False))

        # if h_layers == 0:
        #     self.layers.append(dglnn.CuGraphGATConv(in_feats=in_feats, out_feats=out_feats, num_heads=1, bias=False, activation=F.relu))
        # else:
        #     self.layers.append(dglnn.CuGraphGATConv(in_feats=in_feats, out_feats=n_hidden, num_heads=1, bias=False, activation=F.relu))
        #     for layer in range(h_layers - 1):
        #         self.layers.append(dglnn.CuGraphGATConv(in_feats=n_hidden, out_feats=n_hidden, num_heads=1, bias=False, activation=F.relu))
        #     self.layers.append(dglnn.CuGraphGATConv(in_feats=n_hidden, out_feats=out_feats,  num_heads=1, bias=False))
...
def full_trainer(graph,
    ...
    with graph.local_scope():
        ...
        for epoch in range(n_epochs):
            ...
            if train_model:
                # print(train_mask.shape, logits.shape, labels.shape)
        ...
        # for i_l in range(h_layers + 1):
        #     trained_weights = model_weights['layers.' + str(i_l) + '.weight'].cpu().numpy()
        #     print(trained_weights.shape, trained_weights)
        #     export_dense_mm(trained_weights.astype(np.float32), "/W" + str(i_l) + "train.npy", weight_path)
