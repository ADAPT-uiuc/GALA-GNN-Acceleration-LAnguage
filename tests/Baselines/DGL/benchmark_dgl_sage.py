...
class GCNOpt(nn.Module):
    def __init__(self,
        ...
        if h_layers == 0:
            self.layers.append(dglnn.SAGEConv(in_feats, out_feats, "mean", bias=False, activation=F.relu))
        else:
            self.layers.append(dglnn.SAGEConv(in_feats, n_hidden, "mean", bias=False, activation=F.relu))
            for layer in range(h_layers - 1):
                self.layers.append(dglnn.SAGEConv(n_hidden, n_hidden, "mean", bias=False, activation=F.relu))
            self.layers.append(dglnn.SAGEConv(n_hidden, out_feats, "mean", bias=False))
