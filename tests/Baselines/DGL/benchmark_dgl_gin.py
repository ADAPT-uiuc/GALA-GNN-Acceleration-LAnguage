...
class GCNOpt(nn.Module):
    def __init__(self,
        ...
        if h_layers == 0:
            mlp = nn.Linear(in_feats, out_feats)
            self.layers.append(dglnn.GINConv(ApplyNodeFunc(mlp), "sum", init_eps=0, learn_eps=True, activation=F.relu))
        else:
            mlp_in = nn.Linear(in_feats, n_hidden)
            self.layers.append(dglnn.GINConv(ApplyNodeFunc(mlp_in), "sum", init_eps=0, learn_eps=True, activation=F.relu))
            for layer in range(h_layers - 1):
                mlp_hid = nn.Linear(n_hidden, n_hidden)
                self.layers.append(dglnn.GINConv(ApplyNodeFunc(mlp_hid), "sum", init_eps=0, learn_eps=True, activation=F.relu))
            mlp_out = nn.Linear(n_hidden, out_feats)
            self.layers.append(dglnn.GINConv(ApplyNodeFunc(mlp_out), "sum", init_eps=0, learn_eps=True))
