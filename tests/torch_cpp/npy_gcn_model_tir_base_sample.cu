...
torch::Tensor gather_forward_gcn(torch::Tensor input_dense,
                                 ...
                                 torch::Tensor value_graph, int ra, int rb) {
  ...
  if ((int)dcols / 64) {
    ...
    if ((dcols % 64) > 32) {
      ...
      default_function_kernel32_offset_sample<<<gridDim_rem, blockDim_rem, 0,
                                                stream2>>>(
          oden_array, offset_ptr, val_ptr, iden_ptr, col_ptr, nrows, dcols,
...
struct GCN : torch::nn::Module {
  ...
  forward(torch::Tensor input_dense,   // B
    ...
    torch::Tensor degree = torch::full({nrows, 1}, sample_size, options);
...
int main(int argc, char **argv) {
  ...
  for (size_t epoch = 1; epoch <= num_iters; ++epoch) {
    ...
    torch::Tensor prediction =
        net->forward(t_iden, t_offsets, t_cols, t_vals, nrows, sample_size)[0];
