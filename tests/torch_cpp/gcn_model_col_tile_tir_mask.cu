...
extern "C" __global__ void __launch_bounds__(256)
    default_function_kernel64_undir(float *__restrict__ C,
                                    ...
                                    bool *__restrict__ J_mask, int nrows,
                                    int dcols) {
  if (((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) < nrows) {
    for (int j = 0;
      ...
      C[((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
         ...
         ((int)threadIdx.x))] =
          (C[((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
               (((int)blockIdx.y) * 64)) +
              ((int)threadIdx.x))] +
           (J_mask[(j + J_indptr_data[((((int)blockIdx.x) * 8) +
                                       ((int)threadIdx.y))])]) *
               (B[(((J_indices_data[(j +
                                     J_indptr_data[((((int)blockIdx.x) * 8) +
                                                    ((int)threadIdx.y))])] *
                     dcols) +
                    (((int)blockIdx.y) * 64)) +
                   ((int)threadIdx.x))]));
      C[(((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
           (((int)blockIdx.y) * 64)) +
          ((int)threadIdx.x)) +
         32)] = (C[(((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
                      (((int)blockIdx.y) * 64)) +
                     ((int)threadIdx.x)) +
                    32)] +
                 (J_mask[(j + J_indptr_data[((((int)blockIdx.x) * 8) +
                                             ((int)threadIdx.y))])]) *
                     (B[((((J_indices_data[(
                                j + J_indptr_data[((((int)blockIdx.x) * 8) +
                                                   ((int)threadIdx.y))])] *
                            dcols) +
                           (((int)blockIdx.y) * 64)) +
                          ((int)threadIdx.x)) +
  ...
  if (((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) < nrows) {
    for (int j = 0;
      ...
      C[((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
          (((int)blockIdx.y) * 32)) +
         ((int)threadIdx.x)) +
        offset] = (C[((((((int)blockIdx.x) * 8) + ((int)threadIdx.y)) * dcols +
                       (((int)blockIdx.y) * 32)) +
                      ((int)threadIdx.x)) +
                     offset] +
                   (J_mask[(j + J_indptr_data[((((int)blockIdx.x) * 8) +
...
std::vector<at::Tensor> gather_forward_gcn(torch::Tensor input_dense,
  ...
  auto options_bool = torch::TensorOptions()
                          .dtype(torch::kBool)
                          .requires_grad(false)
                          .device(torch::kCUDA, 0);
  auto mask_tensor = torch::ones({nvals}, options_bool);
    ...
    if (is_directed) {
      ...
      if ((int)dcols / 64) {
        ...
        default_function_kernel64_undir<<<gridDim, blockDim, 0, stream1>>>(
            ...
            &col_ptr[start_vals], &mask[start_vals], nrows, dcols);
        ...
        if ((int)dcols / 32) {
          ...
          default_function_kernel_rem_undir<<<gridDim_rem, blockDim_rem, 0,
                                              stream1>>>(
              oden_array, &offset_ptr[i1 * (nrows + 1)], iden_ptr,
              &col_ptr[start_vals], &mask[start_vals], nrows, dcols, 0);
        }
      }
    }
  }
