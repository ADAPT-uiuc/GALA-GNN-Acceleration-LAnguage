...
struct GIN : torch::nn::Module {
  GIN(int in_size, int hidden_size, int out_size, bool dir, float init_eps) {
    ...
    eps1 = register_parameter("eps1", torch::tensor({init_eps}));
    eps2 = register_parameter("eps2", torch::tensor({init_eps}));
  ...
  forward(torch::Tensor input_dense,   // B
    ...
    torch::Tensor res_n = GatherForward::apply(res, offset_graph, columns_graph, value_graph,
                               bounds);
    res = (1 + eps1) * res + res_n;
    res = torch::relu(res);
    if (hidden_feat_size > out_feat_size){
      res = fc2->forward(res);
      res_n = GatherForward::apply(res, offset_graph, columns_graph, value_graph,
                                bounds);
  ...
  torch::Tensor eps1{nullptr}, eps2{nullptr};
