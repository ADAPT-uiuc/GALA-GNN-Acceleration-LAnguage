...
int main(int argc, char **argv) {
	...
	associations->push_back(&trgrapgFeatAssociation);
	...
	/// NOTE -- This degree operation is necessary to get the mean of the aggregated results
	/// The front-end would have "aggrFn = dsl.get_aggregate(fn = dsl.fn.mul_mean);" as the aggregation operation,
	/// but it's lowered down into (a) getting the degree values, (b) getting the -1 power of the degrees,
	/// (the ones till now are necessary only once), (c) the mul_sum aggregation, and (d) dividing by degrees to get
	/// the mean.
	/// Degree op
	// Ones
	...
	// Normalization operation (get power -1)
	...
	auto normFeatData = DataNode("res", INT32, INT32, F32, &rootNormFeat2Level);
	normFeat2.addInputData(&normData);
	normFeat2.addInputData(&outputData);
	normFeat2.addOutputData(&normFeatData);
	trainingLoop.addLoopNode(&normFeat2);
	auto normFeat2NormDependency = RelationEdge(&normData, ALL_RELATION, &normFeatData, ROWS_RELATION);
	dependencies->push_back(&normFeat2NormDependency);
	auto normFeat2FeatDependency = RelationEdge(&outputData, ALL_RELATION, &normFeatData, ALL_RELATION);
	dependencies->push_back(&normFeat2FeatDependency);
	auto normFeat2NormFeatAssociation = RelationEdge(&normData, ALL_RELATION, &outputData, ROWS_RELATION);
	...
	auto normFeatData_2 = DataNode("res", INT32, INT32, F32, &rootNormFeat2Level_2);
	normFeat2_2.addInputData(&normData);
	normFeat2_2.addInputData(&outputData_2);
	normFeat2_2.addOutputData(&normFeatData_2);
	trainingLoop.addLoopNode(&normFeat2_2);
	auto normFeat2NormDependency_2 = RelationEdge(&normData, ALL_RELATION, &normFeatData_2, ROWS_RELATION);
	dependencies->push_back(&normFeat2NormDependency_2);
	auto normFeat2FeatDependency_2 = RelationEdge(&outputData_2, ALL_RELATION, &normFeatData_2, ALL_RELATION);
	dependencies->push_back(&normFeat2FeatDependency_2);
	auto normFeat2NormFeatAssociation_2 = RelationEdge(&normData, ALL_RELATION, &outputData_2, ROWS_RELATION);
