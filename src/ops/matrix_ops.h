...
#include "../objects/coo_matrix.h"
#include "../objects/dense_matrix.h"
#include "../objects/matrix_prop.h"
...
template<class DM, class SM, class itype>
void SpMM_fdt(const SM *A, const DM *B, DM *out, itype tiling_step) {
    //Feature dimension tiled version of SpMM. Feature size in all layers must be a multiple of tiling step.
    ...
    //This should be decided be the compiler.
...
#pragma omp parallel for schedule(guided)
    for (iT v = 0; v < A_rows; v++) {
        ...
        dnT jj = 0;
        for (dnT jj = 0; jj < B_cols - B_cols % tiling_step; jj += tiling_step) {
            //for(dnT jj = 0; jj < B_cols; jj += tiling_step){
...
#pragma omp simd
                for (dnT j = jj; j < jj + tiling_step; j++) {
                    base1[j] += A_val * base2[j];
                }
            }
        }
    }
...
void tile_jj_iip_i_j_kv(std::vector<SM *> adj_vec,
                        DM *inp_dense,
                        DM *out_dense,
