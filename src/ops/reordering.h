...
//                iT u = adj_ids_ptr[e];
////                std::cout << i_i << " " << u << " " << adj->nvals() << std::endl;
...
//    new_dense_adj->build(nrows, ncols, d_nvals, d_row_ids, d_col_ids, d_vals, adj->type());
//
//    SM *new_sparse_adj = new SM;
//    new_sparse_adj->build(nrows, ncols, s_nvals, s_row_ids, s_col_ids, s_vals, adj->type());
//
...
template<class SM>
void rowReorder(SM *adj, typename SM::itype *perm) {
    // Sparse input (Graph)
    typedef typename SM::itype iT;
    typedef typename SM::ntype nT;
    typedef typename SM::vtype vT;
 ...
 */
template<class DM>
void rowPermuteDenseFrom(DM *&src,
                         typename DM::itype *perm) {
    ...
    // TODO create a copy of the current values, and then shift using the values in the copied array to the new
    diT input_nrows = src->nrows();
    diT input_ncols = src->ncols();
    dvT *input_old_vals = (dvT *) malloc((input_ncols * input_nrows) * sizeof(dvT));
    dvT *input_vals_ptr = src->vals_ptr();

    std::copy(src->vals_ptr(), src->vals_ptr() + (input_ncols * input_nrows), input_old_vals);
    ...
    for (diT i_i = 0; i_i < input_nrows; i_i++) {
        ...
        for (diT i_j = 0; i_j < input_ncols; i_j++) {
            input_vals_ptr[new_row_offset + i_j] = input_old_vals[old_row_offset + i_j];
        }
    }
    free(input_old_vals);
}
...
void rowPermuteDenseTo(DM *&src,
    ...
    diT input_nrows = src->nrows();
    diT input_ncols = src->ncols();
    dvT *input_new_vals = (dvT *) aligned_alloc(64, (input_ncols * input_nrows) * sizeof(dvT));
    dvT *input_vals_ptr = src->vals_ptr();
...
#pragma omp parallel for schedule(static, 4)
    for (diT i_i = 0; i_i < input_nrows; i_i++) {
        ...
        for (diT i_j = 0; i_j < input_ncols; i_j++) {
            input_new_vals[new_row_offset + i_j] = input_vals_ptr[old_row_offset + i_j];
        }
    }
//    src->clear();
#ifdef ICC_CMP
    //    std::copy(pstl::execution::par_unseq, input_new_vals, input_new_vals + (input_ncols * input_nrows), input_vals_ptr);
        std::copy(input_new_vals, input_new_vals + (input_ncols * input_nrows), input_vals_ptr);
#else
    std::copy(input_new_vals, input_new_vals + (input_ncols * input_nrows), input_vals_ptr);
#endif
...
template<class SM>
void getMinOneBlockPerm(std::vector<SM *> adjs,
...
void rowReorder(std::vector<SM *> adjs,
                DM_d *input,
                DM_i *train_mask,
                DM_i *valid_mask,
                DM_i *test_mask,
...
#pragma omp parallel
        for (iT i = 0; i < adj->nrows(); i += 1) {
            ...
            // TODO if reordered, then need to check first not last, irrespective of jj order
            if (has_min_one) {
#ifdef REV_JJ
...
#pragma omp parallel for schedule(dynamic) default(none) shared(adj, new_offset_ptr, new_ids_ptr, new_vals_ptr, perm, adj_offset_ptr, adj_ids_ptr, adj_vals_ptr)
    for (iT i = 0; i < adj->nrows(); i += 1) {
        ...
        // TODO This is WRONG. You are doing for move tos with the perm[id[e]], also no need of the -1s here
        for (nT e = old_pre_offset; e < old_post_offset; e++) {
            if (adj_ids_ptr[e] == -1) {
                local_id_vals[e - old_pre_offset].first = adj_ids_ptr[e];
                has_min_one = true;
...
void rowReorderTo(SM *adj,
    ...
    double time_init, time_sparse_serial, time_sparse_parallel, time_dense, time_others, time_total;
    start_time = get_time();
...
#pragma omp parallel for schedule(dynamic) default(none) shared(adj, new_offset_ptr, new_ids_ptr, new_vals_ptr, perm, adj_offset_ptr, adj_ids_ptr, adj_vals_ptr)
    ...
    std::cout << "Init: " << time_init << " , percen: " << time_init * 100 / time_total
              << std::endl;
    std::cout << "Sparse serial: " << time_sparse_serial << " , percen: " << time_sparse_serial * 100 / time_total
              << std::endl;
    std::cout << "Sparse parallel: " << time_sparse_parallel << " , percen: "
              << time_sparse_parallel * 100 / time_total
              << std::endl;
    std::cout << "Dense: " << time_dense << " , percen: " << time_dense * 100 / time_total
              << std::endl;
    std::cout << "Others: " << time_others << " , percen: " << time_others * 100 / time_total
              << std::endl;
#endif
}
 ...
 * Columns not listed will be in previously arranged order AFTER the columns listed
 */
template<class SM>
void colReorder(SM *adj, typename SM::itype perm_len, typename SM::itype *perm) {
    // Sparse input (Graph)
    typedef typename SM::itype iT;
    typedef typename SM::ntype nT;
    typedef typename SM::vtype vT;
...
template<class SM>
void getAcendingOrder(SM *adj, typename SM::itype *ret) {
    typedef typename SM::itype iT;
...
template<class SM>
void getDecendingOrder(SM *adj, typename SM::itype *ret) {
    typedef typename SM::itype iT;
