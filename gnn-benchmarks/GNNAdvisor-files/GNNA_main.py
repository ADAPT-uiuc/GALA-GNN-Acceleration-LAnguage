...
if args.model == 'gcn':
    class Net(torch.nn.Module):
        ...
        def forward(self):
            ...
            torch.cuda.synchronize()
            forward_pass_times.append(time.perf_counter() - forward_start)
            return x
else:
    class Net(torch.nn.Module):
        def __init__(self):
...
def train():
    ...
    optimizer.step()
    torch.cuda.synchronize()
    backward_pass_times.append(time.perf_counter() - backward_start)
...
if __name__ == '__main__':
    ...
    forward_time = np.mean(np.array(forward_pass_times[10:]))
    backward_time = np.mean(np.array(backward_pass_times[10:]))
    print('Avg Epoch Time (ms): {:.3f}'.format(train_time*1e3/args.num_epoches))
    print('Avg Forward Time (ms): {:.3f}'.format(forward_time*1e3))
    print('Avg Backward Time (ms): {:.3f}'.format(backward_time*1e3))
